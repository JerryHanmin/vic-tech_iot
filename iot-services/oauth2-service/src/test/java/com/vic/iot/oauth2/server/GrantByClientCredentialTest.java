...
public class GrantByClientCredentialTest extends OAuth2Test {
    ...
    public void accessWithUnknownClientID() throws JsonParseException, JsonMappingException, IOException {
        ResponseEntity<String> response = new TestRestTemplate("trusted-app", "secrets").postForEntity("http://localhost:" + port + "/security/oauth/token?client_id=trusted-app&grant_type=client_credentials", null, String.class);
    }
    ...
    public void accessProtectedResourceByJwtToken() throws JsonParseException, JsonMappingException, IOException, InvalidJwtException {
        ResponseEntity<String> response = new TestRestTemplate().getForEntity("http://localhost:" + port + "/security/resources/client", String.class);
        assertEquals(HttpStatus.UNAUTHORIZED, response.getStatusCode());

        response = new TestRestTemplate("trusted-app", "secret").postForEntity("http://localhost:" + port + "/security/oauth/token?client_id=trusted-app&grant_type=client_credentials", null, String.class);
        String responseText = response.getBody();
        assertEquals(HttpStatus.OK, response.getStatusCode());
        HashMap jwtMap = new ObjectMapper().readValue(responseText, HashMap.class);
        String accessToken = (String) jwtMap.get("access_token");
        ...
        response = new TestRestTemplate().exchange("http://localhost:" + port + "/security/resources/trusted_client", HttpMethod.GET, new HttpEntity<>(null, headers), String.class);
        assertEquals(HttpStatus.OK, response.getStatusCode());
